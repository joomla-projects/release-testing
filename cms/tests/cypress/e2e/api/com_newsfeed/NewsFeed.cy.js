describe('Test that newsfeed API endpoint', () => {
  ['joomla.org'].forEach((file) => {
    it(`can deliver a list of feeds from ${file}`, () => {
      cy.db_createNewsFeed({ name: 'automated test feed', link: `${Cypress.config('baseUrl')}/tests/cypress/data/com_newsfeeds/${file}.xml` })
    ...
    it('can create a feed', () => {
      cy.db_createCategory({ extension: 'com_newsfeeds' })
        .then((categoryId) => cy.api_post('/newsfeeds/feeds', {
          ...
          alias: 'test-feed',
          link: `${Cypress.config('baseUrl')}/tests/cypress/data/com_newsfeeds/${file}.xml`,
          catid: categoryId,
          published: 1,
          language: '*',
          metadesc: '',
    ...
    it('can update a feed', () => {
      cy.db_createNewsFeed({ name: 'automated test contact', access: 1, link: `${Cypress.config('baseUrl')}/tests/cypress/data/com_newsfeeds/${file}.xml` })
    ...
    it('can delete a feed', () => {
      cy.db_createNewsFeed({
        name: 'automated test contact', access: 1, link: `${Cypress.config('baseUrl')}/tests/cypress/data/com_newsfeeds/${file}.xml`, published: -2,
      })
